...
import { DETECTION_FILES } from "@/lib/detections-catalog";
...
function DetectionsPage() {
  ...
  return (
    <Page
      title="Rules"
      lead="Files under windows/detections/. Sysmon XML and markdown are in-repo. Sigma, KQL, and the LOLDrivers gap CSV are not."
    >
      <Notice>
        Merge the Sysmon fragment into an existing config. Do not paste it over a production policy
        without a ring. Do not exclude Signed=true on DriverLoad.
      </Notice>
          ...
          {present.map((file) => (
            <li key={file.id} className="space-y-1">
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Badge variant={file.kind === "sysmon" ? "info" : "mute"}>{file.kind}</Badge>
              </div>
